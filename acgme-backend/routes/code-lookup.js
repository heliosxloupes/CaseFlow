// routes/code-lookup.js
// Ported from api/code-lookup.js (Vercel) — educational CPT / ICD-10 lookup via Claude Haiku
...
router.post('/', async (req, res) => {
  try {
    ...
    const requested = (req.body?.system || 'auto').toLowerCase();
    ...
    const msg = await client.messages.create({
      ...
      max_tokens: 800,
      messages: [{ role: 'user', content: buildPrompt(query, detectedSystem) }],
    });
